...
#CONFIGURABLE: SEE DEFINITION
inputs = world.create_entity(Input({
    "up":[pg.K_UP],
    "down":[pg.K_DOWN],
    "left":[pg.K_LEFT],
    "right":[pg.K_RIGHT]
...
#CONFIGURABLE: SEE DEFINITION
consts = world.create_entity(Consts(
    32,            #TILE SIZE
    (640,480)      #SCREEN SIZE
))
...
camera = world.create_entity(Camera(0,0))
#CONFIGURABLE: SEE DEFINITION
testMap = world.create_entity(TileMap(
    ...
    [
        Tile(pg.image.load("grass.png"), False, world.component_for_entity(consts, Consts)),
        Tile(pg.image.load("shallow.png"), False, world.component_for_entity(consts, Consts))
    ]
))
#CONFIGURABLE: SEE DEFINITION
player = world.create_entity(Position(0,0), 
                             SpriteRenderer(pg.image.load("player.png")),
                            PlayerMove(1))

bobTheNpc = world.create_entity(Position(64,64),
                               SpriteRenderer(pg.image.load("npc.png")))

#END CONFIGURABLE
world.add_processor(InputProcessor(), priority=10)
world.add_processor(PlayerProcessor(), priority=5)
world.add_processor(GraphicsProcessor(), priority=0)
